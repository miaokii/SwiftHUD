...
/// 延迟关闭计时器
@property (nonatomic, weak) NSTimer * exceptionDelayHideTimer;
/// 延迟关闭按钮
@property (nonatomic, strong) MBProgressHUDRoundedButton * button;
...
- (void)showAnimated:(BOOL)animated {
    ...
    // 推迟显示取消按钮
    if (self.exceptionDelayHideSec < 0) {
        self.button.hidden = YES;
    } else if (self.exceptionDelayHideSec == 0) {
        self.button.hidden = NO;
    } else {
        [self.exceptionDelayHideTimer invalidate];
        self.button.hidden = YES;
        NSTimer *timer = [NSTimer timerWithTimeInterval:self.exceptionDelayHideSec target:self selector:@selector(handleExceptionDelayHideTimer:) userInfo:nil repeats:NO];
        [[NSRunLoop currentRunLoop] addTimer:timer forMode:NSRunLoopCommonModes];
        self.exceptionDelayHideTimer = timer;
    }
...
- (void)handleExceptionDelayHideTimer:(NSTimer *)theTimer {
    self.button.hidden = NO;
}
...
- (void)setupViews {
    ...
    [stackView setContentCompressionResistancePriority:998.f forAxis:UILayoutConstraintAxisHorizontal];
    [stackView setContentCompressionResistancePriority:998.f forAxis:UILayoutConstraintAxisVertical];
    ...
    MBProgressHUDRoundedButton *button = [MBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeIndeterminate) {
        if (!isActivityIndicator) {
            ...
            [self.stackView insertArrangedSubview:indicator atIndex:0];
        }
    }
...
- (void)updateConstraints {
    ...
    [stackConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(==margin)-[stackView]-(==margin)-|" options:0 metrics:metrics views:NSDictionaryOfVariableBindings(stackView)]];
    [stackConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(==margin)-[stackView]-(==margin)-|" options:0 metrics:metrics views:NSDictionaryOfVariableBindings(stackView)]];
    [self applyPriority:999.f toConstraints:stackConstraints];
    [bezel addConstraints:stackConstraints];
...
- (void)updatePaddingConstraints {
    self.indicator.hidden = self.mode == MBProgressHUDModeText;
